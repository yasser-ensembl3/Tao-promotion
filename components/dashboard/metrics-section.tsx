...
export function MetricsSection() {
  ...
  const handleCardClick = (metricType: string) => {
    if (selectedMetricType === metricType) {
      ...
      // Autre carte = sélectionner et ouvrir
      setSelectedMetricType(metricType)
      setExpanded(true)
    }
  }

  // Fetch metrics from Notion with 60s cache
  ...
  const handleSaveMetric = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`Failed to create metric: ${errorData.error}`)
      }
    } catch (error: any) {
  ...
  const keyMetrics = latestMetrics.length > 0 ? (
    ...
    <div className="text-center p-6 rounded-lg bg-muted/50 border border-dashed">
      <p className="text-sm text-muted-foreground">No metrics yet. Add your first metric to get started.</p>
    </div>
  )
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <h4 className="font-semibold">Metrics Chart</h4>
                ...
                return (
                  <div className="p-8 border rounded-lg text-center border-dashed">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Click &ldquo;Add Metric&rdquo; to create your first metric
                    </p>
                  </div>
                )
              }
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input
                id="metric-type"
                placeholder="e.g., Number of sales, Monthly subscribers"
